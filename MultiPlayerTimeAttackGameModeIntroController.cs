namespace TrickyMultiplayerPlus
{
    ...
    internal class MultiPlayerTimeAttackGameModeIntroController : AbstractMultiPlayerGameModeIntroController, IHUDInjectable, IGameModelInjectable, IGameDataInjectable, IBrickPickerInjectable, IInjectable
    {
        ...
        {
            if (!this._brickPickers.Contains(brickPicker)) { this._brickPickers.Add(brickPicker); }
        }

        public void SetHud(AbstractHUD hud)
        {
            if (!this._huds.Contains(hud)) { this._huds.Add(hud); }
        }

        public void SetGameData(GameData gameData)
        {
            if (!this._gameData.Contains(gameData)) { this._gameData.Add(gameData); }
        }
        ...
        public void SetGameModel(GameModel gameModel)
        {
            ...
            if (!this._brickLeftModels.Contains(dataModel)) { this._brickLeftModels.Add(dataModel); }
            if (!this._nextBrickModels.Contains(dataModel2)) { this._nextBrickModels.Add(dataModel2); }
        }
